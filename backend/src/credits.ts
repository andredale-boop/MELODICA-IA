...
export type CreditKind = 'PURCHASE'|'RESERVE'|'SETTLE'|'REFUND'|'BONUS'|'ADJUSTMENT';
...
async function ledger(client: PoolClient, userId: string, amount: number, kind: CreditKind, referenceId: string, metadata={}) {
  const id = randomUUID();
...
export async function reserveCreditsTx(c: PoolClient, userId: string, amount: number, referenceId: string) {
  ...
  if (!r.rowCount) throw new Error('INSUFFICIENT_CREDITS');
  return r.rows[0];
}

export async function reserveCredits(pool: Pool, userId: string, amount: number, referenceId: string) {
  const c = await pool.connect();
  try { await c.query('BEGIN'); const result = await reserveCreditsTx(c,userId,amount,referenceId); await c.query('COMMIT'); return result; }
  catch(e) { await c.query('ROLLBACK'); throw e; } finally { c.release(); }
}

export async function settleCreditsTx(c: PoolClient, userId: string, reservedAmount: number, actualAmount: number, referenceId: string) {
  if (reservedAmount < 0 || actualAmount < 0 || actualAmount > reservedAmount) throw new Error('INVALID_SETTLEMENT');
  ...
  if (!r.rowCount) throw new Error('RESERVATION_NOT_FOUND');
  await ledger(c,userId,-actualAmount,'SETTLE',referenceId,{reservedAmount,actualAmount});
  if (refund) await ledger(c,userId,refund,'REFUND',referenceId,{reservedAmount,actualAmount});
  return r.rows[0];
}

export async function settleCredits(pool: Pool, userId: string, reservedAmount: number, actualAmount: number, referenceId: string) {
  const c = await pool.connect();
  try { await c.query('BEGIN'); const result = await settleCreditsTx(c,userId,reservedAmount,actualAmount,referenceId); await c.query('COMMIT'); return result; }
  catch(e) { await c.query('ROLLBACK'); throw e; } finally { c.release(); }
}

export async function grantVerifiedPurchase(pool: Pool, userId: string, productId: string, purchaseToken: string) {
  const amount = (PLAY_PRODUCTS as Record<string, number>)[productId];
  if (!amount || purchaseToken.length < 16) throw new Error('INVALID_PURCHASE');
  const c = await pool.connect();
  try {
    await c.query('BEGIN');
    await c.query('INSERT INTO credit_accounts(user_id) VALUES($1) ON CONFLICT DO NOTHING', [userId]);
    ...
    if (inserted.rowCount) await c.query('UPDATE credit_accounts SET balance=balance+$2,updated_at=now() WHERE user_id=$1', [userId, amount]);
