...
app.get('/health',(_req,res)=>res.json({ok:true,service:'melodica-api',version:'1.3.0'}));
...
app.delete('/v1/account',auth,async(req:Req,res)=>{if(!pool)return res.status(503).json({error:'DATABASE_NOT_CONFIGURED'});await pool.query('DELETE FROM users WHERE id=$1',[req.userId]);res.status(204).end()});
...
app.get('/v1/projects',auth,async(req:Req,res)=>{if(!pool)return res.status(503).json({error:'DATABASE_NOT_CONFIGURED'});const r=await pool.query('SELECT id,title,style,created_at,updated_at FROM projects WHERE user_id=$1 ORDER BY updated_at DESC',[req.userId]);res.json(r.rows)});
app.post('/v1/projects',auth,async(req:Req,res)=>{const p=projectSchema.safeParse(req.body);if(!p.success)return res.status(400).json({error:'INVALID_REQUEST'});if(!pool)return res.status(503).json({error:'DATABASE_NOT_CONFIGURED'});const r=await pool.query('INSERT INTO projects(user_id,title,style) VALUES($1,$2,$3) RETURNING *',[req.userId,p.data.title,p.data.style]);res.status(201).json(r.rows[0])});
...
async function runJob(jobId:string){
 if(!pool)return; const r=await pool.query('SELECT * FROM jobs WHERE id=$1',[jobId]); if(!r.rowCount)return; const j=r.rows[0];
 try{
  await pool.query("UPDATE jobs SET status='RUNNING',updated_at=now() WHERE id=$1 AND status='QUEUED'",[jobId]);
  ...
  if(providerGateway){
   ...
   if(!resp.ok)throw new Error(`PROVIDER_HTTP_${resp.status}`); const data:any=await resp.json();
   if(data.assetPath) assetPath=String(data.assetPath); else if(data.assetUrl){assetPath=String(data.assetUrl)} else throw new Error('PROVIDER_RESPONSE_MISSING_ASSET');
  } else if(process.env.ALLOW_LOCAL_DEMO==='true' && !j.mode.toLowerCase().includes('video')) {
   const filename=`${jobId}.wav`; await writeFile(path.join(assetsDir,filename),makeWav(8)); assetPath=filename;
  } else { throw new Error('AI_PROVIDER_NOT_CONFIGURED'); }
  ...
  await pool.query("UPDATE jobs SET status='SUCCEEDED',asset_url=$2,settled_credits=$3,completed_at=now(),updated_at=now() WHERE id=$1",[jobId,assetUrl,j.reserved_credits]);
 }catch(e:any){
  await pool.query("UPDATE jobs SET status='FAILED',error=$2,completed_at=now(),updated_at=now() WHERE id=$1 AND status<>'CANCELED'",[jobId,String(e?.message??'GENERATION_FAILED').slice(0,500)]).catch(()=>{});
  await settleCredits(pool,j.user_id,j.reserved_credits,0,`fail-${jobId}`).catch(()=>{});
 }
}

app.post('/v1/generations',auth,async(req:Req,res)=>{const p=generationSchema.safeParse(req.body);if(!p.success)return res.status(400).json({error:'INVALID_REQUEST',details:p.error.flatten()});if(!pool)return res.status(503).json({error:'DATABASE_NOT_CONFIGURED'});const cost=(CREDIT_COSTS as Record<string,number>)[p.data.mode];if(!cost)return res.status(400).json({error:'UNKNOWN_GENERATION_MODE'});try{await reserveCredits(pool,req.userId!,cost,p.data.idempotencyKey);const job=await pool.query(`INSERT INTO jobs(user_id,project_id,mode,prompt,status,reserved_credits,idempotency_key) VALUES($1,$2,$3,$4,'QUEUED',$5,$6) ON CONFLICT(user_id,idempotency_key) DO UPDATE SET updated_at=now() RETURNING id,status,reserved_credits`,[req.userId,p.data.projectId,p.data.mode,p.data.prompt,cost,p.data.idempotencyKey]);res.status(202).json({id:job.rows[0].id,status:job.rows[0].status,progress:0,reservedCredits:job.rows[0].reserved_credits});setImmediate(()=>runJob(job.rows[0].id))}catch(e:any){if(e?.message==='INSUFFICIENT_CREDITS')return res.status(402).json({error:'INSUFFICIENT_CREDITS'});res.status(409).json({error:'CREDIT_RESERVATION_FAILED'})}});
...
app.get('/v1/assets/:jobId',auth,async(req:Req,res)=>{
 ...
 if(!existsSync(file))return res.status(404).json({error:'ASSET_FILE_NOT_FOUND'});
 res.sendFile(file);
});
app.post('/v1/generations/:id/cancel',auth,async(req:Req,res)=>{if(!pool)return res.status(503).json({error:'DATABASE_NOT_CONFIGURED'});const r=await pool.query(`UPDATE jobs SET status='CANCELED',updated_at=now() WHERE id=$1 AND user_id=$2 AND status IN ('QUEUED','RUNNING') RETURNING id,status,reserved_credits`,[req.params.id,req.userId]);if(!r.rowCount)return res.status(404).json({error:'JOB_NOT_FOUND_OR_NOT_CANCELABLE'});await settleCredits(pool,req.userId!,r.rows[0].reserved_credits,0,`cancel-${r.rows[0].id}`);res.status(202).json({status:'CANCELED',id:r.rows[0].id})});
