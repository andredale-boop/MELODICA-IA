...
app.post('/v1/billing/verify',auth,async(req:Req,res)=>{
 const p=purchaseSchema.safeParse(req.body); if(!p.success)return res.status(400).json({error:'INVALID_PURCHASE_REQUEST'});
 if(!pool)return res.status(503).json({error:'DATABASE_NOT_CONFIGURED'});
 const verifier=process.env.GOOGLE_PLAY_VERIFIER_URL; if(!verifier)return res.status(503).json({error:'PLAY_VERIFIER_NOT_CONFIGURED'});
 try {
  ...
  if(!response.ok)return res.status(502).json({error:'PLAY_VERIFIER_UNAVAILABLE'});
  const verified=await response.json() as {verified?:boolean;productId?:string};
  if(verified.verified!==true||verified.productId!==p.data.productId)return res.status(402).json({error:'PURCHASE_NOT_VERIFIED'});
 ...
 } catch { return res.status(502).json({error:'PURCHASE_VERIFICATION_FAILED'}); }
});
...
async function finalizeJob(pool: Pool, jobId: string, outcome: 'SUCCEEDED'|'FAILED', errorMessage?: string) {
